<script lang="ts">
    ...
    function changeModel(name: string) {
        DBState.db.aiModel = name;
        close();
    }
</script>

<!-- svelte-ignore a11y_click_events_have_key_events -->
<div class="fixed top-0 w-full h-full left-0 bg-black/50 z-50 flex justify-center items-center" role="button" tabindex="0" onclick={close}>
    <div class="w-96 max-w-full max-h-full overflow-y-auto overflow-x-hidden bg-bgcolor p-4 flex flex-col" role="button" tabindex="0" onclick={(e) => e.stopPropagation()}>
        ...
        <div class="text-textcolor2 text-sm mb-2">
            {getModelInfo(DBState.db.aiModel)?.fullName || language.none}
        </div>
