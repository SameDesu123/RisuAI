...
import { normalizeBotUiConfig, type BotUiConfigV1 } from '../process/botUiConfig';
export { normalizeBotUiConfig, type BotUiConfigV1 } from '../process/botUiConfig';
...
export function setDatabase(data:Database){
    ...
    for(const entry of data.characters){
        if(entry?.type === 'group' || !entry?.botUi){
            continue
        }
        const botUi = normalizeBotUiConfig(entry.botUi)
        if(!botUi){
            delete entry.botUi
            continue
        }
