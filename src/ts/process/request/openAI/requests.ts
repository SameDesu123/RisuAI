...
import { applyAdditionalParameters, applyParameters, getAdditionalParameters, setObjectValue } from '../shared'
...
export async function requestOpenAI(arg:RequestDataArgumentExtended):Promise<requestDataResponse>{
    ...
    if(arg.useStreaming){
        ...
        da.body.pipeTo(transtream.writable, { preventAbort: true }).catch(async () => {
...
function getTranStream(arg:RequestDataArgumentExtended):TransformStream<Uint8Array, StreamResponseChunk> {
    ...
    const buildEnqueueValue = ():{[key:string]:string} => {
        let result = {...readed}

        if(arg.modelInfo.flags.includes(LLMFlags.deepSeekThinkingOutput) && result["0"]){
            result["0"] = result["0"].replace(/(.*)<\/think>/gms, (m, p1) => {
                reasoningContent = p1
                return ""
            })
        ...
        if(arg.extractJson && (db.jsonSchemaEnabled || arg.schema)){
            ...
            for(const key in result){
                const extracted = extractJSON(result[key], arg.extractJson)
    ...
    return new TransformStream<Uint8Array, StreamResponseChunk>({
        transform(chunk, control) {
            ...
            try {
                parseLines(lines)
                control.enqueue(buildEnqueueValue())
            } catch (error) {
