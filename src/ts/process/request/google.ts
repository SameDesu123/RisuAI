...
async function requestGoogle(url:string, body:any, headers:{[key:string]:string}, arg:RequestDataArgumentExtended):Promise<requestDataResponse> {
    ...
    const processTextResponse = (rDatas: {text: string, thought?: boolean}[]) => {
        ...
        for (const d of rDatas) {
            (d.thought ? thoughtParts : contentParts).push(d.text)
        }
            ...
            if (line.startsWith('data: ')) {
                ...
                try {
                            ...
                            if (part.text) {
                                ...
                                if (part.thoughtSignature) {
                                    ...
                                    if(saveSignature){
                                        ...
                                        accumulated["0"] += `{{inlayeddata::${sigId}}}`;
                                    }
                                }
                            }
                    ...
                    if(jsonData.usageMetadata){
                        accumulated['__usageMetadata'] = JSON.stringify(jsonData.usageMetadata)
                    }
                    if(jsonData.modelStatus){
                        accumulated['__modelStatus'] = JSON.stringify(jsonData.modelStatus)
                    }
                } catch (error) {
