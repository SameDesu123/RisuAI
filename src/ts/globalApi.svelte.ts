...
export async function preLoadDatabaseBlockChat(characterIndex: number, chatIndex: number) {
    await hydrateDatabaseBlockChat(DBState.db, getDatabaseBlockStorage(), characterIndex, chatIndex)
}
...
export async function decodeDatabaseStorageBytes(data: Uint8Array) {
    if (isDatabaseBlockManifest(data)) {
        return await loadDatabaseBlockDatabase(data, getDatabaseBlockStorage())
    }
...
export async function saveDb() {
    ...
    if (!getDatabase().databaseBlockStorage) {
    ...
    while (true) {
        ...
        try {
            ...
            if (db.databaseBlockStorage) {
                const blockStorage = getDatabaseBlockStorage()
                const dbData = await saveDatabaseBlockDatabase(db, blockStorage, toSave)
                if (!forageStorage.isAccount) {
                    await blockStorage.setItem(`database/dbbackup-${(Date.now() / 100).toFixed()}.bin`, dbData)
                    await getDbBackups()
                }
                if (forageStorage.isAccount) {
                    await sleep(3000)
